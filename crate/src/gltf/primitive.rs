use crate::{
    ...
    renderer::shaders::{ShaderKey, SkinTarget},
};
use anyhow::bail;
...
impl AwsmRenderer {
    pub(super) fn add_gltf_primitive(
        ...
        let data_to_add = if let Some(prim_entity) = prim_entity {
            ...
            let mut color_map:BTreeMap<u32, u32> = BTreeMap::new();
            ...
            for (semantic, accessor) in primitive.attributes() {
                match semantic {
                    ...
                    Semantic::Colors(color_index) => {
                        ...
                        color_map.insert(color_index, dynamic_loc);
                    },
            ...
            if !color_map.is_empty() {
                shader_key.vertex_colors = Some(color_map.into_values().collect());
            }
